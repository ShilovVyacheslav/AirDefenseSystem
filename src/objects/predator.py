...
class Predator(Entity):
    def __init__(self, pos, vel=(0, 0), behavior=None):
        super().__init__(pos, vel, radius_world=config.PREDATOR_RADIUS_WORLD,
    ...
    def draw(self, screen, scale, offset):
        ...
        config.pygame.draw.circle(screen, self.color, (int(p_screen.x), int(p_screen.y)), r_px, 1)
        config.pygame.draw.circle(screen, self.color, (int(p_screen.x), int(p_screen.y)), 1)
        if self.engagement_status == "TRACK":
            config.pygame.draw.circle(screen, self.color, (int(p_screen.x), int(p_screen.y)), r_px * 1.5, 1)
        elif self.engagement_status == "ENGAGE":
            config.pygame.draw.circle(screen, self.color, (int(p_screen.x), int(p_screen.y)), r_px * 1.5, 1)
            config.pygame.draw.circle(screen, self.color, (int(p_screen.x), int(p_screen.y)), r_px * 2.0, 1)
        status_text = config.font_small.render(f"P-{self.track_id}", True, self.color)
    ...
    def has_captured(self, target):
        distance = config.pygame.Vector2(self.pos).distance_to(target.pos)

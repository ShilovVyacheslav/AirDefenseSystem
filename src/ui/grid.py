...
def draw_grid(screen, W, H, scale, offset):
    ...
    center_screen = world_to_screen(config.pygame.Vector2(0, 0), scale, offset)
    for r in range(1, 6):
        radius_px = r * 100
        config.pygame.draw.circle(screen, config.COLOR_GRID_MINOR,
    ...
    for i in range(0, 5000):
        ...
        if x > x_max:
            break
        xs = world_to_screen(config.pygame.Vector2(x, 0), scale, offset).x
        color = config.COLOR_GRID_MINOR if abs(x) > 1e-9 else config.COLOR_AXIS
        config.pygame.draw.line(screen, color, (xs, 0), (xs, H), 1)
    ...
    for i in range(0, 5000):
        ...
        if y > y_max:
            break
        ys = world_to_screen(config.pygame.Vector2(0, y), scale, offset).y

...
class MatrixOverlay:
    ...
    def draw(self, screen, entity_manager):
        ...
        targets = entity_manager.targets

        if not predators or not targets:
            return
        ...
        max_cols = min(8, len(targets))

        for j in range(max_cols):
            target = targets[j]
            header_text = f"T-{target.track_id}"
        ...
        for i in range(max_rows):
            for j in range(max_cols):
                ...
                time_text = "∞" if pursuit_time == float('inf') else f"{pursuit_time:.1f}s"
                if (i, j) in entity_manager.assignment:
